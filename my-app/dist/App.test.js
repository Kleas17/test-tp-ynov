...
beforeEach(() => {
  ...
  apiModule.createRegistration.mockResolvedValue({
    id: 1
  });
});
...
describe('Navigation SPA et formulaire - integration', () => {
  ...
  test('email deja utilise: erreur visible et soumission bloquee', async () => {
    apiModule.getRegistrations.mockResolvedValueOnce([{
      ...userValide
    }]);
    await renderApp();
  ...
  test('erreur metier backend 400: affiche le message du serveur', async () => {
    apiModule.createRegistration.mockRejectedValueOnce({
      response: {
        status: 400,
        data: {
          message: 'Cet email est déjà utilisé (back)'
        }
      }
    });
    await renderApp();
  ...
  test('erreur metier backend 409: affiche le message du serveur', async () => {
    apiModule.createRegistration.mockRejectedValueOnce({
      response: {
        status: 409,
        data: {
          message: 'Conflit: email déjà existant (409)'
        }
      }
    });
    await renderApp();
    ...
    await waitFor(() => expect(window.location.pathname).toBe('/register'));
    expect(await screen.findByRole('status')).toHaveTextContent('Conflit: email déjà existant (409)');
  });
  test('erreur serveur 500: affiche une alerte utilisateur resilient', async () => {
    ...
    await waitFor(() => expect(window.location.pathname).toBe('/register'));
    expect(await screen.findByRole('status')).toHaveTextContent('Serveur indisponible, veuillez réessayer plus tard.');
  });
